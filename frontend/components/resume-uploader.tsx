...
import { FileText, X, Check, AlertCircle, Loader2, FileUp, File } from "lucide-react"
...
export function ResumeUploader({ onClose }: ResumeUploaderProps) {
  ...
  const handleUpload = useCallback(() => {
    ...
    // Simulate upload progress with exponential slowdown for realism
    let progress = 0
    const simulateUpload = () => {
      ...
      if (progress < 99) {
        ...
        setTimeout(() => {
          ...
          try {
            const existingFiles = JSON.parse(localStorage.getItem("uploadedResumes") || "[]")
            localStorage.setItem("uploadedResumes", JSON.stringify([...existingFiles, file.name]))
          } catch (err) {
            console.error("Error storing file in localStorage:", err)
          }
        }, 500)
      }
    }
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        ...
        {!file ? (
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 border rounded-lg bg-muted/10">
              ...
              <Button variant="ghost" size="icon" onClick={resetUpload} disabled={isUploading} aria-label="Remove file">
                <X className="h-4 w-4" />
              </Button>
            </div>
        ...
        {uploadedFiles.length > 0 && (
          <div className="mt-4">
            <h4 className="text-sm font-medium mb-2">Previously uploaded resumes:</h4>
